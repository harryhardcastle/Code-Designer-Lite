...
namespace CodeDesignerLite
{
    public partial class Form1 : Form
    {
        ...
        private string configOutputFormat = "PS2";
        private int configFontSize = 10;
        private string configCodeFormat = "-";
        private bool suppressConfigSave = false;
        private bool configSaveWarningShown = false;
        ...
        private class EditorTab
        {
            ...
            public int InputFirstVisibleLine { get; set; }
            public int InputXOffset { get; set; }
            public int InputCurrentPosition { get; set; }
            public int InputAnchorPosition { get; set; }
            public Panel TabPanel { get; set; }
            public Label TitleLabel { get; set; }
            public Button CloseButton { get; set; }

            public EditorTab(string filePath, string text, int untitledNumber)
            {
                FilePath = filePath;
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            ...
            {
                if (!ConfirmCloseTab(editorTabs[i]))
                {
                    e.Cancel = true;
                    return;
                }
        ...
        private void ApplyDarkTitleBar()
        {
            try
            {
                if (Environment.OSVersion.Platform != PlatformID.Win32NT || this.Handle == IntPtr.Zero)
                    return;

                int useDarkMode = 1;
                // Windows 10 20H1+ uses 20; older Windows 10 builds use 19. Try both safely.
            ...
            {
                // Older Windows builds simply keep the default title bar.
            }
        }
        ...
        private void ConfigureDarkRadioButton(RadioButton radioButton)
        {
            ...
            DarkRadioButton darkRadioButton = radioButton as DarkRadioButton;
            if (darkRadioButton != null)
            {
                darkRadioButton.UseVisualStyleBackColor = false;
                darkRadioButton.BackColor = colorPaneBackground;
                darkRadioButton.ForeColor = colorTextSecondary;
                darkRadioButton.RadioFillColor = Color.FromArgb(24, 26, 29);
                darkRadioButton.RadioBorderColor = colorControlHover;
                darkRadioButton.RadioCheckedColor = colorAccent;
                darkRadioButton.RadioHoverFillColor = Color.FromArgb(30, 33, 36);
                darkRadioButton.TextColor = colorTextSecondary;
                darkRadioButton.TextCheckedColor = colorTextPrimary;
                return;
            }

            radioButton.UseVisualStyleBackColor = false;
            radioButton.FlatStyle = FlatStyle.Flat;
            radioButton.BackColor = colorPaneBackground;
            radioButton.ForeColor = colorTextSecondary;
            radioButton.FlatAppearance.BorderColor = colorControlBackground;
            radioButton.FlatAppearance.CheckedBackColor = colorEditorBackground;
            radioButton.FlatAppearance.MouseOverBackColor = colorControlHover;
            radioButton.FlatAppearance.MouseDownBackColor = colorControlHover;
        }

        private void EnsureDarkRadioButtonPreferredSize(RadioButton radioButton)
        {
            if (radioButton == null)
                return;

            Size preferredSize = radioButton.GetPreferredSize(Size.Empty);
            radioButton.AutoSize = false;
            radioButton.MinimumSize = new Size(preferredSize.Width, Math.Max(22, preferredSize.Height));
            radioButton.Size = radioButton.MinimumSize;
        }
        ...
        private void DarkComboBox_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            Color backColor = selected ? colorControlHover : colorControlBackground;
            ...
            {
                string text = Convert.ToString(comboBox.Items[e.Index], CultureInfo.InvariantCulture);
        ...
        private void RtbInput_TextChanged(object sender, EventArgs e)
        {
            ...
            if (activeTab != null)
            {
                activeTab.Text = rtbInput.Text;
                activeTab.IsDirty = true;
                UpdateTabVisual(activeTab);
                UpdateWindowTitle();
            }

            errorLineNumbersToHighlight.Clear();
            RefreshInputHorizontalScrollWidth();
        ...
        private void InitializeComponentManual()
        {
            ...
            Panel leftPane = new Panel { Dock = DockStyle.Fill, BackColor = colorPaneBackground, Padding = Padding.Empty, Margin = Padding.Empty };
            mainPanel.Controls.Add(leftPane, 0, 0);

            lblInput = new Label { Text = "INPUT", AutoSize = true, ForeColor = colorTextSecondary, Margin = new Padding(3, 5, 3, 3), Anchor = AnchorStyles.Left | AnchorStyles.Bottom };

            // --- Font Size Controls ---
            lblFontSize = new Label { Text = "FONT SIZE:", AutoSize = true, ForeColor = colorTextSecondary, Margin = new Padding(5, 5, 0, 3) };
            cmbFontSize = new DarkComboBox
            {
            ...
            DarkComboBox darkFontSizeCombo = cmbFontSize as DarkComboBox;
            if (darkFontSizeCombo != null)
            {
                darkFontSizeCombo.DarkBackColor = colorControlBackground;
                darkFontSizeCombo.DarkForeColor = colorTextPrimary;
                darkFontSizeCombo.DarkBorderColor = colorControlHover;
                darkFontSizeCombo.DarkButtonColor = colorControlBackground;
                darkFontSizeCombo.DarkButtonHoverColor = colorControlHover;
            }
            for (int i = 5; i <= 36; i++)
            {
                cmbFontSize.Items.Add(i);
            }
            if (cmbFontSize.Items.Contains(configFontSize))
                cmbFontSize.SelectedItem = configFontSize;
            else
                cmbFontSize.SelectedItem = 10; // Default font size
            cmbFontSize.DrawItem += DarkComboBox_DrawItem;
            cmbFontSize.HandleCreated += (s, e) =>
            {
                ApplyDarkThemeToControl(cmbFontSize);
                DarkComboBox fontSizeCombo = cmbFontSize as DarkComboBox;
                if (fontSizeCombo != null)
                    fontSizeCombo.ApplyDarkThemeToDropDownList();
            };
            ...
            txtAddressFormat = new TextBox { Text = configCodeFormat, Width = 30, MaxLength = 1, TextAlign = HorizontalAlignment.Center, BackColor = colorControlBackground, ForeColor = colorTextPrimary, BorderStyle = BorderStyle.None, Margin = new Padding(0, 3, 5, 0) };
            txtAddressFormat.TextChanged += TxtAddressFormat_TextChanged;

            TableLayoutPanel inputHeaderPanel = new TableLayoutPanel { Dock = DockStyle.Top, Height = 30, RowCount = 1, ColumnCount = 6, AutoSize = false, BackColor = colorPaneBackground, Padding = Padding.Empty, Margin = Padding.Empty };
            inputHeaderPanel.RowStyles.Add(new RowStyle(SizeType.Absolute, 30F));
            ...
            rbPS2Mode = new DarkRadioButton
            {
                Text = "PS2",
                AutoSize = true,
                ForeColor = colorTextSecondary,
                BackColor = colorPaneBackground, // Match panel background
                Checked = string.Equals(configOutputFormat, "PS2", StringComparison.OrdinalIgnoreCase),
                Margin = new Padding(0, 3, 5, 0) // (left, top, right, bottom)
            };
            rbPnachMode = new DarkRadioButton
            {
                Text = "pnach",
                AutoSize = true,
                ForeColor = colorTextSecondary,
                BackColor = colorPaneBackground,
                Checked = string.Equals(configOutputFormat, "pnach", StringComparison.OrdinalIgnoreCase) || string.Equals(configOutputFormat, "Pnach", StringComparison.OrdinalIgnoreCase),
                Margin = new Padding(5, 3, 5, 0) // Add some spacing
            };
            ConfigureDarkRadioButton(rbPS2Mode);
            ConfigureDarkRadioButton(rbPnachMode);
            EnsureDarkRadioButtonPreferredSize(rbPS2Mode);
            EnsureDarkRadioButtonPreferredSize(rbPnachMode);
            if (!rbPS2Mode.Checked && !rbPnachMode.Checked)
                rbPS2Mode.Checked = true;
            currentOutputFormat = rbPnachMode.Checked ? OutputFormatMode.Pnach : OutputFormatMode.PS2;
            ...
            Panel rightPane = new Panel { Dock = DockStyle.Fill, BackColor = colorPaneBackground, Padding = Padding.Empty, Margin = Padding.Empty };
            ...
            lblOutputLineCount = new Label { Text = "LINES: 0", AutoSize = true, ForeColor = colorTextSecondary, TextAlign = ContentAlignment.MiddleRight, Margin = new Padding(3, 5, 3, 3), Anchor = AnchorStyles.Right | AnchorStyles.Bottom };

            TableLayoutPanel outputHeaderPanel = new TableLayoutPanel { Dock = DockStyle.Top, Height = 30, RowCount = 1, ColumnCount = 2, AutoSize = false, BackColor = colorPaneBackground, Padding = Padding.Empty, Margin = Padding.Empty };
            outputHeaderPanel.RowStyles.Add(new RowStyle(SizeType.Absolute, 30F));
            outputHeaderPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50F));
            outputHeaderPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50F));
            lblOutput.Anchor = AnchorStyles.Left | AnchorStyles.Bottom;
            ...
            outputAreaLayout.RowStyles.Add(new RowStyle(SizeType.Percent, 100F));
            outputAreaLayout.RowStyles.Add(new RowStyle(SizeType.AutoSize));
            ...
            ApplyDarkThemeToControl(rtbInput);
            ApplyDarkThemeToControl(rtbOutput);
        ...
        private void TxtAddressFormat_TextChanged(object sender, EventArgs e)
        {
            configCodeFormat = NormalizeCodeFormat(txtAddressFormat != null ? txtAddressFormat.Text : configCodeFormat);
            SaveUserConfig();
        }

        private void LoadUserConfig()
        {
            userConfigPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, UserConfigFileName);
            configOutputFormat = "PS2";
            configFontSize = 10;
            configCodeFormat = "-";
            ...
            try
            {
                ...
                {
                    foreach (string rawLine in File.ReadAllLines(userConfigPath))
                    {
                        string line = rawLine.Trim();
                        if (line.Length == 0 || line.StartsWith("#"))
                            continue;

                        Match match = Regex.Match(line, @"^([A-Za-z0-9_]+)\s*=\s*""?(.*?)""?\s*$");
                        if (!match.Success)
                            continue;
                        ...
                        if (string.Equals(key, "OutputFormat", StringComparison.OrdinalIgnoreCase))
                        {
                            configOutputFormat = NormalizeOutputFormat(value);
                        }
                        else if (string.Equals(key, "FontSize", StringComparison.OrdinalIgnoreCase))
                        {
                            int parsedFontSize;
                            if (int.TryParse(value, NumberStyles.Integer, CultureInfo.InvariantCulture, out parsedFontSize))
                                configFontSize = ClampToRange(parsedFontSize, 5, 36);
                        }
                        else if (string.Equals(key, "CodeFormat", StringComparison.OrdinalIgnoreCase))
                        {
                            configCodeFormat = NormalizeCodeFormat(value);
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine("Could not read user config: " + ex.Message);
            }

            WriteUserConfigFile(configOutputFormat, configFontSize, configCodeFormat);
        }
        ...
        private void SaveUserConfig()
        {
            ...
            string outputFormat = currentOutputFormat == OutputFormatMode.Pnach ? "pnach" : "PS2";
            int fontSize = configFontSize;
            if (cmbFontSize != null && cmbFontSize.SelectedItem != null)
            {
                if (cmbFontSize.SelectedItem is int)
                    fontSize = (int)cmbFontSize.SelectedItem;
                else
                {
                    int parsedFontSize;
                    if (int.TryParse(Convert.ToString(cmbFontSize.SelectedItem, CultureInfo.InvariantCulture), NumberStyles.Integer, CultureInfo.InvariantCulture, out parsedFontSize))
                        fontSize = parsedFontSize;
                }
            }

            string codeFormat = NormalizeCodeFormat(txtAddressFormat != null ? txtAddressFormat.Text : configCodeFormat);

            configOutputFormat = NormalizeOutputFormat(outputFormat);
            configFontSize = ClampToRange(fontSize, 5, 36);
            configCodeFormat = codeFormat;
            WriteUserConfigFile(configOutputFormat, configFontSize, configCodeFormat);
        }

        private string NormalizeOutputFormat(string value)
        {
            if (string.Equals(value, "pnach", StringComparison.OrdinalIgnoreCase) || string.Equals(value, "Pnach", StringComparison.OrdinalIgnoreCase))
                return "pnach";
            return "PS2";
        }

        private string NormalizeCodeFormat(string value)
        {
            if (value == null)
                return "-";
            if (value.Length > 1)
        ...
        private void WriteUserConfigFile(string outputFormat, int fontSize, string codeFormat)
        {
            ...
            try
            {
                string safeCodeFormat = (codeFormat ?? string.Empty).Replace("\"", string.Empty);
                string[] lines = new string[]
                {
                    "OutputFormat=\"" + NormalizeOutputFormat(outputFormat) + "\"",
                    "FontSize=\"" + ClampToRange(fontSize, 5, 36).ToString(CultureInfo.InvariantCulture) + "\"",
                    "CodeFormat=\"" + safeCodeFormat + "\""
                };
                File.WriteAllLines(userConfigPath, lines, new UTF8Encoding(false));
            }
            catch (Exception ex)
            {
                Console.WriteLine("Could not write user config: " + ex.Message);
                if (!configSaveWarningShown && IsHandleCreated)
                {
                    configSaveWarningShown = true;
                    MessageBox.Show("Could not save CodeDesignerLite.config: " + ex.Message, "Config Save Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        }
        ...
        private void UpdateLineCount()
        {
            ...
            if (activeTab != null && !isSwitchingTabs)
            {
                activeTab.OutputText = rtbOutput.Text;
            }
        }


        private Image CreateOpenTabButtonImage()
        {
            Bitmap image = new Bitmap(16, 16);
            using (Graphics g = Graphics.FromImage(image))
            using (SolidBrush folderFill = new SolidBrush(colorTextPrimary))
            using (Pen outlinePen = new Pen(colorTextPrimary, 1.2F))
            {
                g.SmoothingMode = SmoothingMode.AntiAlias;
                g.Clear(Color.Transparent);

                Rectangle tabRect = new Rectangle(2, 4, 6, 3);
                Rectangle bodyRect = new Rectangle(2, 6, 12, 8);
                g.FillRectangle(folderFill, tabRect);
                g.DrawRectangle(outlinePen, tabRect);
                g.DrawRectangle(outlinePen, bodyRect);

                using (SolidBrush subtleBrush = new SolidBrush(Color.FromArgb(60, colorTextPrimary)))
                {
                    g.FillRectangle(subtleBrush, new Rectangle(bodyRect.Left + 1, bodyRect.Top + 1, bodyRect.Width - 1, bodyRect.Height - 1));
                }
            }
        ...
        private Panel CreateTabBarPanel()
        {
            ...
            tabFlowPanel.Resize += (s, e) => ResizeTabsToFit();

            btnOpenTab = new Button
            {
                Text = string.Empty,
                Image = CreateOpenTabButtonImage(),
            ...
            btnOpenTab.FlatAppearance.BorderSize = 0;
            btnOpenTab.FlatAppearance.MouseOverBackColor = colorControlHover;
            btnOpenTab.Click += BtnOpen_Click;
            ...
            btnAddTab.FlatAppearance.MouseOverBackColor = colorControlHover;
            btnAddTab.Click += (s, e) => CreateNewTab(null, string.Empty, true);

            panel.Controls.Add(tabFlowPanel);
            return panel;
        }

        private void ConfigureMenuStrip(MenuStrip strip)
        {
            strip.BackColor = colorMenuBackground;
            strip.ForeColor = colorTextPrimary;
            strip.RenderMode = ToolStripRenderMode.Professional;
            strip.Renderer = new ToolStripProfessionalRenderer(new CustomColorTable(colorMenuBackground, colorMenuHover, colorControlBackground, colorAccent));
            strip.Padding = new Padding(4, 2, 0, 2);

            foreach (ToolStripItem item in strip.Items)
            {
                ApplyToolStripDarkTheme(item);
            }
        }

        private void ApplyToolStripDarkTheme(ToolStripItem item)
        {
            item.ForeColor = colorTextPrimary;
            item.BackColor = colorMenuBackground;

            ToolStripMenuItem menuItem = item as ToolStripMenuItem;
            if (menuItem != null)
            {
                menuItem.DropDown.BackColor = colorMenuBackground;
                menuItem.DropDown.ForeColor = colorTextPrimary;
                menuItem.DropDown.Padding = Padding.Empty;
                foreach (ToolStripItem child in menuItem.DropDownItems)
                {
                    if (child is ToolStripSeparator)
                    {
                        child.BackColor = colorMenuBackground;
                    }
                    else
                    {
                        ApplyToolStripDarkTheme(child);
                    }
                }
            }
        }

        private void CreateNewTab(string filePath, string text, bool switchToTab)
        {
            EditorTab tab = new EditorTab(filePath, text, 0);
            editorTabs.Add(tab);
        ...
        private void CreateTabControl(EditorTab tab)
        {
            Panel panel = new Panel
            {
                Height = 28,
            ...
            closeButton.FlatAppearance.MouseOverBackColor = colorMenuHover;

            panel.MouseDown += (s, e) => HandleTabMouseDown(tab, e);
            titleLabel.MouseDown += (s, e) => HandleTabMouseDown(tab, e);
        ...
        private void HandleTabMouseDown(EditorTab tab, MouseEventArgs e)
        {
        ...
        private void ResizeTabsToFit()
        {
            if (tabFlowPanel == null || btnAddTab == null || editorTabs.Count == 0)
                return;

            int tabButtonWidth = btnAddTab.Width + btnAddTab.Margin.Horizontal;
            if (btnOpenTab != null)
            ...
            {
                int fittedWidth = availableWidth / editorTabs.Count;
                if (fittedWidth < tabWidth)
                    tabWidth = fittedWidth;
            }
            ...
            foreach (EditorTab tab in editorTabs)
            {
                if (tab.TabPanel != null)
                {
                    tab.TabPanel.Width = tabWidth;
                    tab.TabPanel.Height = 28;
                    UpdateTabVisual(tab);
                }
            }
        }
        ...
        private void SwitchToTab(EditorTab tab)
        {
            ...
            try
            {
                ...
                rtbInput.Text = activeTab.Text ?? string.Empty;
                rtbOutput.Text = activeTab.OutputText ?? string.Empty;
                RefreshInputHorizontalScrollWidth();
            }
            finally
            {
            ...
            UpdateLineCount();
            HighlightSyntax(0, rtbInput.TextLength);
            RestoreInputViewState(activeTab);
        }

        private void ClearEditorForNoTabs()
        {
            isSwitchingTabs = true;
            try
            {
                ...
                if (rtbOutput != null)
                {
                    ...
                    rtbOutput.BackColor = colorEditorBackground;
                    rtbOutput.ForeColor = colorTextPrimary;
                }
            }
            finally
            {
        ...
        {
            if (activeTab == null || rtbInput == null || rtbOutput == null)
                return;
        ...
        {
            if (activeTab == null || rtbInput == null || rtbInput.IsDisposed)
                return;

            try
            {
                int textLength = rtbInput.TextLength;
                activeTab.InputFirstVisibleLine = Math.Max(0, rtbInput.FirstVisibleLine);
                activeTab.InputXOffset = Math.Max(0, rtbInput.XOffset);
                activeTab.InputCurrentPosition = ClampToRange(rtbInput.CurrentPosition, 0, textLength);
                activeTab.InputAnchorPosition = ClampToRange(rtbInput.AnchorPosition, 0, textLength);
            }
            catch
            {
                // If Scintilla is mid-dispose or temporarily unavailable, keep the last saved view state.
            }
        }

        private void RefreshInputHorizontalScrollWidth()
        {
            if (rtbInput == null || rtbInput.IsDisposed)
                return;

            try
            {
                int previousXOffset = Math.Max(0, rtbInput.XOffset);
                int calculatedWidth = CalculateInputHorizontalScrollWidth();
                rtbInput.ScrollWidth = 1;
                rtbInput.ScrollWidth = calculatedWidth;
                rtbInput.XOffset = previousXOffset;
            }
            catch
            {
                // Horizontal scroll recalculation should never interrupt editing or tab changes.
            }
        }

        private int CalculateInputHorizontalScrollWidth()
        {
            if (rtbInput == null || rtbInput.IsDisposed || rtbInput.TextLength == 0)
                return 1;

            int widestLineWidth = 1;
            foreach (ScintillaNET.Line line in rtbInput.Lines)
            {
                ...
                int lineWidth = rtbInput.TextWidth(ScintillaNET.Style.Default, lineText);
                if (lineWidth > widestLineWidth)
                    widestLineWidth = lineWidth;
            }

            return Math.Max(1, widestLineWidth + 24);
        }

        private void RestoreInputViewState(EditorTab tab)
        {
            if (tab == null || rtbInput == null || rtbInput.IsDisposed)
                return;

            try
            {
                int textLength = rtbInput.TextLength;
                int anchorPosition = ClampToRange(tab.InputAnchorPosition, 0, textLength);
                int currentPosition = ClampToRange(tab.InputCurrentPosition, 0, textLength);
                int maxVisibleLine = Math.Max(0, rtbInput.Lines.Count - 1);
                int firstVisibleLine = ClampToRange(tab.InputFirstVisibleLine, 0, maxVisibleLine);

                rtbInput.AnchorPosition = anchorPosition;
                rtbInput.CurrentPosition = currentPosition;
                RefreshInputHorizontalScrollWidth();
                rtbInput.XOffset = Math.Max(0, tab.InputXOffset);

                if (firstVisibleLine != rtbInput.FirstVisibleLine)
                    rtbInput.FirstVisibleLine = firstVisibleLine;
            }
            catch
            {
                // Restoring view state is a convenience; editing should continue even if the state is invalid.
            }
        }
        ...
        private void CloseTab(EditorTab tab)
        {
            ...
            int closedIndex = editorTabs.IndexOf(tab);
            bool wasActive = tab == activeTab;
            editorTabs.Remove(tab);

            if (tabFlowPanel != null && tab.TabPanel != null)
                tabFlowPanel.Controls.Remove(tab.TabPanel);
            if (tab.TabPanel != null)
            ...
            {
                int nextIndex = closedIndex;
                if (nextIndex >= editorTabs.Count)
                    nextIndex = editorTabs.Count - 1;
                SwitchToTab(editorTabs[nextIndex]);
            }
        ...
        private bool ConfirmCloseTab(EditorTab tab)
        {
            ...
            DialogResult result = MessageBox.Show("Save changes to " + tab.DisplayName + " before closing?",
                                                  "Unsaved Changes",
        ...
        private void UpdateTabVisual(EditorTab tab)
        {
            if (tab == null || tab.TabPanel == null || tab.TitleLabel == null)
                return;

            bool selected = tab == activeTab;
            tab.TabPanel.BackColor = selected ? colorControlBackground : colorFormBackground;
            tab.TitleLabel.ForeColor = selected ? colorTextPrimary : colorTextSecondary;
            if (tab.CloseButton != null)
            {
                tab.CloseButton.ForeColor = selected ? colorTextPrimary : colorTextSecondary;
                tab.CloseButton.BackColor = tab.TabPanel.BackColor;
            }

            int reservedWidth = 34;
            int textWidth = tab.TabPanel.Width - reservedWidth;
            if (textWidth < 12)
                textWidth = 12;

            string title = tab.IsDirty ? "*" + tab.DisplayName : tab.DisplayName;
            tab.TitleLabel.Text = TruncateTextToWidth(title, tab.TitleLabel.Font, textWidth);
            tab.TabPanel.Tag = title;
            tab.TabPanel.Invalidate();
        }

        private string TruncateTextToWidth(string text, Font font, int maxWidth)
        {
            ...
            while (keep > 1)
            {
                string candidate = text.Substring(0, keep) + ellipsis;
        ...
        {
            CreateNewTab(null, string.Empty, true);
        }
        ...
        private void BtnOpen_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    EditorTab existingTab = editorTabs.FirstOrDefault(t => string.Equals(t.FilePath, selectedFile, StringComparison.OrdinalIgnoreCase));
                    if (existingTab != null)
                    {
                    ...
                    // Read the file using Windows-1252 encoding to preserve byte-oriented cheat/assembly text.
                    string fileText = File.ReadAllText(selectedFile, Encoding.GetEncoding("Windows-1252"));
                    CreateNewTab(selectedFile, fileText, true);
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(activeTab.FilePath))
            {
                try
                {
                    File.WriteAllText(activeTab.FilePath, rtbInput.Text, Encoding.GetEncoding("ISO-8859-1"));
                    activeTab.Text = rtbInput.Text;
                    activeTab.IsDirty = false;
                    currentFilePath = activeTab.FilePath;
                    Console.WriteLine($"Saved to {activeTab.FilePath}");
                    UpdateTabVisual(activeTab);
                    UpdateWindowTitle();
                }
                catch (Exception ex)
        ...
        private void BtnSaveAs_Click(object sender, EventArgs e)
        {
            using (SaveFileDialog saveFileDialog = new SaveFileDialog())
            {
                ...
                string suggestedName = activeTab != null && !string.IsNullOrEmpty(activeTab.FilePath) ? Path.GetFileName(activeTab.FilePath) : "untitled.cds";
        ...
        private class DarkRadioButton : RadioButton
        {
            ...
            public Color RadioFillColor { get; set; }
            public Color RadioBorderColor { get; set; }
            public Color RadioCheckedColor { get; set; }
            public Color RadioHoverFillColor { get; set; }
            public Color TextColor { get; set; }
            public Color TextCheckedColor { get; set; }

            public DarkRadioButton()
            {
                SetStyle(ControlStyles.UserPaint | ControlStyles.AllPaintingInWmPaint | ControlStyles.OptimizedDoubleBuffer | ControlStyles.ResizeRedraw, true);
                RadioFillColor = Color.FromArgb(35, 39, 42);
                RadioBorderColor = Color.FromArgb(79, 84, 92);
                RadioCheckedColor = Color.FromArgb(88, 101, 196);
                RadioHoverFillColor = Color.FromArgb(42, 46, 50);
                TextColor = Color.FromArgb(185, 187, 190);
                TextCheckedColor = Color.FromArgb(220, 221, 222);
            }

            public override Size GetPreferredSize(Size proposedSize)
            {
                Size textSize = TextRenderer.MeasureText(Text ?? string.Empty, Font);
                int width = 1 + 13 + 5 + textSize.Width + 6;
                int height = Math.Max(22, Math.Max(13 + 4, textSize.Height + 4));
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                Rectangle radioRect = new Rectangle(1, top, diameter, diameter);

                using (SolidBrush fillBrush = new SolidBrush(isMouseOver ? RadioHoverFillColor : RadioFillColor))
                using (Pen borderPen = new Pen(RadioBorderColor))
                {
                    e.Graphics.FillEllipse(fillBrush, radioRect);
                    e.Graphics.DrawEllipse(borderPen, radioRect);
                }

                if (Checked)
                {
                    Rectangle dotRect = new Rectangle(radioRect.Left + 4, radioRect.Top + 4, radioRect.Width - 8, radioRect.Height - 8);
                    using (SolidBrush checkedBrush = new SolidBrush(RadioCheckedColor))
                    {
                        e.Graphics.FillEllipse(checkedBrush, dotRect);
                    }
                }

                Rectangle textRect = new Rectangle(radioRect.Right + 5, 0, Width - radioRect.Right - 5, Height);
                TextRenderer.DrawText(e.Graphics, Text, Font, textRect, Checked ? TextCheckedColor : TextColor, TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPrefix);

                if (Focused && ShowFocusCues)
                    ControlPaint.DrawFocusRectangle(e.Graphics, textRect, Checked ? TextCheckedColor : TextColor, BackColor);
            }
        }
        ...
        private class DarkComboBox : ComboBox
        {
            ...
            public Color DarkForeColor { get; set; }
            public Color DarkBorderColor { get; set; }
            public Color DarkButtonColor { get; set; }
            public Color DarkButtonHoverColor { get; set; }

            public DarkComboBox()
            {
                DarkBackColor = Color.FromArgb(70, 74, 80);
                DarkForeColor = Color.FromArgb(220, 221, 222);
                DarkBorderColor = Color.FromArgb(79, 84, 92);
                DarkButtonColor = Color.FromArgb(70, 74, 80);
                DarkButtonHoverColor = Color.FromArgb(79, 84, 92);
            ...
            {
                base.OnHandleCreated(e);
                ApplyDarkThemeToDropDownList();
            }

            protected override void OnDropDown(EventArgs e)
            {
                // Do not apply native theme changes while the popup is opening.
                // Calling SetWindowTheme against the live dropdown/list handle here can
                // make the Win32 ComboBox immediately collapse before the user can click.
                base.OnDropDown(e);
            }
            ...
            public void ApplyDarkThemeToDropDownList()
            {
                ...
                try
                {
                    ...
                    comboInfo.cbSize = Marshal.SizeOf(typeof(COMBOBOXINFO));
                    if (GetComboBoxInfo(Handle, ref comboInfo))
                    {
                        if (comboInfo.hwndCombo != IntPtr.Zero)
                            SetWindowTheme(comboInfo.hwndCombo, "DarkMode_Explorer", null);
                        if (comboInfo.hwndItem != IntPtr.Zero)
                            SetWindowTheme(comboInfo.hwndItem, "DarkMode_Explorer", null);
                        if (comboInfo.hwndList != IntPtr.Zero)
                            SetWindowTheme(comboInfo.hwndList, "DarkMode_Explorer", null);
                    }
                }
                catch
                {
                    // The native combo box dropdown is OS-version dependent; keep the owner-drawn dark colors if theming fails.
                }
            }

            private void DrawDarkComboBox()
            {
                if (!IsHandleCreated || Width <= 0 || Height <= 0)
                    return;

                using (Graphics g = Graphics.FromHwnd(Handle))
                using (SolidBrush backBrush = new SolidBrush(DarkBackColor))
                using (SolidBrush buttonBrush = new SolidBrush(isMouseOver || Focused ? DarkButtonHoverColor : DarkButtonColor))
                using (Pen borderPen = new Pen(DarkBorderColor))
                using (SolidBrush arrowBrush = new SolidBrush(DarkForeColor))
                {
                    Rectangle bounds = ClientRectangle;
                    g.FillRectangle(backBrush, bounds);

                    int buttonWidth = SystemInformation.HorizontalScrollBarArrowWidth;
                    Rectangle buttonRect = new Rectangle(bounds.Right - buttonWidth, bounds.Top, buttonWidth, bounds.Height);
                    g.FillRectangle(buttonBrush, buttonRect);
                    g.DrawRectangle(borderPen, new Rectangle(bounds.Left, bounds.Top, bounds.Width - 1, bounds.Height - 1));

                    string text = SelectedItem != null ? Convert.ToString(SelectedItem, CultureInfo.InvariantCulture) : Text;
                    Rectangle textRect = new Rectangle(bounds.Left + 5, bounds.Top + 1, Math.Max(1, bounds.Width - buttonWidth - 8), bounds.Height - 2);
                    TextRenderer.DrawText(g, text, Font, textRect, DarkForeColor, TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis);

                    int arrowX = buttonRect.Left + (buttonRect.Width / 2);
                    int arrowY = buttonRect.Top + (buttonRect.Height / 2) - 1;
        ...
        {
            private readonly Color menuBack;
            private readonly Color menuHover;
            private readonly Color border;
            private readonly Color accent;

            public CustomColorTable(Color menuBack, Color menuHover, Color border, Color accent)
            {
            ...
            public override Color MenuItemPressedGradientMiddle { get { return menuHover; } }
            ...
            public override Color SeparatorLight { get { return menuHover; } }
        }
    }
}